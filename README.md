...
- **🚀 极致并发**: 在执行批量克隆（Clone）时采用多线程并行处理，最大化利用带宽，告别漫长等待。
- **🧠 智能模式感知**: 无需记忆复杂的命令行参数，通过管道（Pipe）和重定向（Redirect）自动推断用户意图。
- **🛡️ 纯净输出**: 严格分离 `stdout` 和 `stderr`。运行日志和进度提示带高亮色彩，而导出的数据文件保持绝对纯净，无任何 ANSI 污染。
- **👁️ 视觉友好**: 专为浅色终端（Light Mode）优化的 ANSI 配色方案，内置进度预告与视觉分隔线，减轻长期面对代码墙的视觉疲劳。
